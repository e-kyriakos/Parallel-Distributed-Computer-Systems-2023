...
void multiplyCSR_OPENMP(Sparse_CSR *A, Sparse_CSR *B, Sparse_CSR *C, int num_threads) {
...
Sparse_CSR graph_minor(Sparse_CSR A, int *C, int num_nodes, int num_clusters){
	...
	switch (parall_case) {
		case 1:
			...
			multiplyCSR_OPENMP(&Rt, &A, &Temp, num_threads);
			//display_csr(Temp, Temp.rows, "Temp");
			multiplyCSR_OPENMP(&Temp, &R, &M, num_threads);
